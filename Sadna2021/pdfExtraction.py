...
def mergeLines(lineToMergeInto, line):
    for i in range(0, len(lineToMergeInto)):
        if lineToMergeInto[i]:
            lineToMergeInto[i] = lineToMergeInto[i] + " " + line[i]
        else:
            return


def mergeSplitTableToLastTable(tables, tableToMerge):
    # merge last line in tables with first line in tableToMerge
    mergeLines(tables[-1][-1], tableToMerge[0])
    # add the rest of lines in tableToMerge to tables.
    tables[len(tables) - 1].extend(tableToMerge[1:])
    ...
    while currPageIndex[0] < len(pdf.pages) and tablesDone is False:
        ...
        for i in range(0, len(tables)):
            ...
            if courseTable:
                ...
                tablesStart = True
            elif tablesStart and pageHasElectiveCourses(page):
                mergeSplitTableToLastTable(mandatoryCoursesTables, tables[i])
                tablesDone = True
